...
export function OnboardingWizard({
  ...
  const sectionIncomplete =
    !!currentSection && currentSection.questions.some((q) => q.required && !answers[q.key]?.trim());
  ...
  if (step === 0) {
    return (
      <div className="mx-auto flex max-w-lg flex-col items-center gap-4 rounded-2xl bg-white p-8 text-center shadow-sm">
        <h1 className="text-2xl text-zinc-900">Bienvenue chez Diabolo Agency, {clientNom} !</h1>
        <p className="text-sm text-zinc-500">
          Avant de démarrer votre projet vidéo, merci de répondre à ce court questionnaire de brief. Il nous permet
          de bien cerner votre offre, votre cible et vos objectifs pour livrer des vidéos qui vous ressemblent.
        </p>
        <p className="text-xs text-zinc-400">{totalSteps} sections · une dizaine de minutes</p>
        <button
          type="button"
          onClick={() => setStep(1)}
  ...
  return (
    <div className="mx-auto flex max-w-xl flex-col gap-6 rounded-2xl bg-white p-8 shadow-sm">
      <div>
        ...
        {currentSection!.questions.map((q) => (
          <label key={q.key} className="flex flex-col gap-1">
            <span className="text-sm font-medium text-zinc-700">
              {q.label}
              {!q.required && <span className="ml-1 font-normal text-zinc-400">(optionnel)</span>}
            </span>
            {q.type === "textarea" ? (
              <textarea
                value={answers[q.key] ?? ""}
      ...
      <div className="flex items-center justify-between">
        ...
        {step < totalSteps ? (
          ...
          <button
            type="button"
            disabled={sectionIncomplete || isPending}
